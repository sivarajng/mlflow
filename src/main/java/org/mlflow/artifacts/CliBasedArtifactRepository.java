...
/**
 * Shells out to the 'mlflow' command line utility to upload, download, and list
 * artifacts. This
 * is used as a fallback to implement any artifact repositories which are not
 * natively supported
 * within Java.
 *
...
public class CliBasedArtifactRepository implements ArtifactRepository {
  ...
  // Global check if we ever successfully loaded 'mlflow'. This allows us to print
  // a more
  ...
  // Base directory of the artifactory, used to let the user know why this
  ...
  /**
   * Only available in the CliBasedArtifactRepository. Downloads an artifact to
   * the local
   * filesystem when provided with an artifact uri. This method should not be used
   * directly
  ...
  private List<Service.FileInfo> parseFileInfos(String json) {
    // The protobuf deserializer doesn't allow us to directly deserialize a list, so
    // we
    // deserialize a list-of-dictionaries, and then reserialize each dictionary to
    // pass it to
  ...
  /**
   * Checks whether the 'mlflow' executable is available, and throws a nice error
   * if not.
   * If this method has ever run successfully before (in the entire JVM), we will
   * not rerun it.
   */
   ...
   * @param tag           User-facing tag which will be used to identify what we
   *                      were trying to do
   *                      in the case of a failure.
   * @return raw stdout of the process, decoded as a utf-8 string
   * @throws MlflowClientException if the process exits with a non-zero exit code,
   *                               or anything
   *                               else goes wrong.
   */
  ...
  /**
   * Appends --run-id $runId and --artifact-path $artifactPath, omitting
   * artifactPath if null.
   */
  ...
  /**
   * Returns user-facing identifier "runId=abc, artifactId=/foo", omitting
   * artifactPath if null.
   */
